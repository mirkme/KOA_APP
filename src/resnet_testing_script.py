...
# --- Add Paths Here ---
OUTPUT_DIR = "/Users/leo/Desktop/BE Project/Model Training/Test_Model/ResNet50_Test/Resnet50_Test_o:p/Attempt"  
...
resnet = tf.keras.models.load_model('/Users/leo/Desktop/BE Project/Model Training/ResNet50/ResNet50_o:p/finetune/Attempt2/model_ResNet50_ft.hdf5')

# Test data

predictions_xception = resnet.predict(test_generator)
score_xception = resnet.evaluate(test_generator, verbose=1)
...
resnet = models.Model(inputs=input_tensor, outputs=output_tensor)

# Grad-Cam

resnet.summary()

# Get the ResNet50 base model from your full model
last_conv_layer = base_model.get_layer("conv5_block3_out")

'''grad_model = tf.keras.models.Model(
    [base_model.input],
    [last_conv_layer.output, resnet.output]
)'''

grad_model = tf.keras.models.Model(
    inputs=resnet.input,
    outputs=[last_conv_layer.output, resnet.output]
)
...
for k, v in tests_data.items():
    ...
    for i in range(min(6, len(v))):
        ...
        y_pred = resnet.predict(img_array, verbose=0)[0]
