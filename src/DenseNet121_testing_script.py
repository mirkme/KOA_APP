...
# --- Add Paths Here ---
OUTPUT_DIR = "/Users/leo/Desktop/BE Project/Model Training/Test_Model/DensNet121_Test/DensNet121_Test_o:p/Attempt"  
...
densenet = tf.keras.models.load_model('/Users/leo/Desktop/BE Project/Model Training/DensNet121/output/model_DenseNet121_ft.hdf5')

# Test data

predictions_densenet = densenet.predict(test_generator)
score_densenet = densenet.evaluate(test_generator, verbose=1)
print('Test loss:', score_densenet[0])
print('Test acc:', score_densenet[1])
...
densenet = models.Model(inputs=input_tensor, outputs=output_tensor)

# Grad-Cam

#densenet.summary()

# Get the densenet base model from your full model
last_conv_layer = base_model.get_layer("conv5_block16_concat")
'''grad_model = tf.keras.models.Model(
    [base_model.input],
    [last_conv_layer.output, densenet.output]
)'''
grad_model = tf.keras.models.Model(
...
# model
densenet.layers[-1].activation = None
...
for k, v in tests_data.items():
    ...
    for i in range(min(6, len(v))):
        ...
        y_pred = densenet.predict(img_array, verbose=0)[0]
